...
#ifndef OBJECTDETECT
 #define DUMP_INTERMEDIATE_DATA_BUFFER_SIZE     (CAM_FRAME_BUFFER_SIZE + PFC_OUTPUT_BUFFER_SIZE + RESIZE_OUTPUT_BUFFER_SIZE + AI_INPUT_BUFFER_SIZE + (AI_NET_OUTPUT_SIZE*4) + BUFF_NAME_STRING_TOTAL_SIZE)
#else
 #define DUMP_INTERMEDIATE_DATA_BUFFER_SIZE     (CAM_FRAME_BUFFER_SIZE + PFC_OUTPUT_BUFFER_SIZE + RESIZE_OUTPUT_BUFFER_SIZE + AI_INPUT_BUFFER_SIZE + (AI_PEOPLEDETECT_NETWORK_PP_MAX_BOXES_LIMIT*sizeof(struct network_pp_outBuffer)) + BUFF_NAME_STRING_TOTAL_SIZE) 
#endif
...
typedef enum
{
  DATA_FORMAT_RAW             = 0x00,
  DATA_FORMAT_BMP             = 0x01,
  DATA_FORMAT_TXT             = 0x02 /*should be used only for the NN outputs*/
} DataFormat_TypeDef;
...
#else
  uint32_t acc_num_object_detected;
#endif
...
typedef struct
{
  ...
  uint32_t src_bpp; /*Source buffer bits per px for images*/
...
typedef struct
{
  ...
  void*                     AppCtxPtr;
  uint32_t                  NonReg_FirstRun; 
  uint32_t                  Total_Char_Name_Size;  
} TestContext_TypeDef;
...
#else
 extern uint8_t dump_output_buff[];
#endif
extern uint8_t validation_output_buff[];
